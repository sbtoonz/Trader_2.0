...
public class CustomTrader : MonoBehaviour
{
    ...
    private Task<NewElementFormat> GetAndSetupElement(ItemDrop? drop, int price, int stack, int invCount)
    {
        ...
        var component = element.Element.transform.Find("name").GetComponent<TMP_Text>();
        component.text = element.Name;
        component.gameObject.AddComponent<Localize>();

        element.Element.transform.Find("price").GetComponent<TMP_Text>().text = price.ToString();



        // element.Element.transform.Find("stack").GetComponent<TMP_Text>().text = stack switch
        // {
        //     > 1 => "x" + stack,
        //     1 => "",
        //     _ => element.Element.transform.Find("stack").GetComponent<TMP_Text>().text
        // };

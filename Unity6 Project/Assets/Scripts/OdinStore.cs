...
public class OdinStore : MonoBehaviour
{
    ...
    public void AddItemToDisplayList(ItemDrop _drop, int stack, int cost)
    {
        ...
        var name = newElement.Element.transform.Find("name").GetComponent<TMP_Text>();
        name.text = newElement.Name;
        name.gameObject.AddComponent<Localize>();

        newElement.Element.transform.Find("price").GetComponent<TMP_Text>().text = cost.ToString();
    ...
    public void Show()
    {
        ...
        {
            Debug.LogError("[OdinStore] m_StorePanel is NULL! Cannot show UI. Check prefab assignment.");
            return;
        }

        Debug.Log($"[OdinStore] Showing store panel. Current inventory count: {_storeInventory.Count}");
        ...
        {
            Debug.LogWarning("[OdinStore] No elements to display. Store is empty!");
        }
